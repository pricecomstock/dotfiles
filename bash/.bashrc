...
#alias starship='ssh pi@192.168.0.16'
alias pccloud='ssh root@198.58.116.26'
pccloudbuild() {
    if [ $# -eq 1 ]
      then
        docker build --tag=$1 .
    else
        echo "Usage: pccloudbuild [NAME]"
    fi
}
...
pccloudpush() {
    if [ $# -eq 2 ]
      then
        docker tag $1 pricecomstock/$1:$2
        docker push pricecomstock/$1:$2
    else
        echo "Usage: pccloudpush [NAME] [TAG]"
    fi
}
...
if [ -f '/home/price/Downloads/google-cloud-sdk/path.bash.inc' ]; then source '/home/price/Downloads/google-cloud-sdk/path.bash.inc'; fi
